<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/masterPage.Master" CodeBehind="admins.aspx.vb" Inherits="Gayandn.admins" %>
...
<%----------------------------adding new records------------------------------%>
  ...
  <%---------------------------------------------end adding -------------%>
        ...
        <asp:SqlDataSource ID="sdsUsers" runat="server" ConnectionString="<%$ ConnectionStrings:Sharaza_dbConnectionString %>"
            ...
            SelectCommand="SELECT        t_sysUsers.sysUser_id, t_sysUsers.f_name, t_sysUsers.l_name, t_sysUsers.role, t_sysUsers.Phone, t_sysUsers.email, t_sysUsers.active, t_city.c_name FROM  t_sysUsers INNER JOIN t_city ON t_sysUsers.city = t_city.c_id WHERE  (t_sysUsers.role = 'admin') AND (sysUser_id <> 4) ORDER BY t_sysUsers.sysUser_id DESC"
           ...
           <asp:SqlDataSource ID="sqldsSetDeleted" ConnectionString="<%$ ConnectionStrings:Sharaza_dbConnectionString %>" runat="server"  UpdateCommand="UPDATE [t_sysUsers] SET [deleted] = @deleted WHERE [sysUser_id] = @sysUser_id">
              <UpdateParameters>

                  <asp:Parameter Name="deleted" Type="int32"  DefaultValue="1"/>
                  <asp:SessionParameter Name="sysUser_id" SessionField="u_id" />
              </UpdateParameters>
           </asp:SqlDataSource>
        ...
        <asp:GridView ID="GridView1" runat="server" CssClass="table table-bordered responsive"  AutoGenerateColumns="False" DataKeyNames="sysUser_id" DataSourceID="sdsUsers" AllowPaging="True">
          ...
          <%--          <EditItemTemplate>
                ...
                <asp:TemplateField ShowHeader="False">
                    <ItemTemplate>
                        <asp:LinkButton ID="LinkButton2" runat="server" CssClass="btn btn-danger btn-sm btn-icon icon-left" CausesValidation="False"   OnClick="LinkButton2_Click"  OnClientClick="try { return confirm('Are you sure you want to delete data'); } catch(e){alert(e.message);}" >
